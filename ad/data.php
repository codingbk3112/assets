...
$url, $use_include_path = false, $context = null, $offset = 0, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) {
    ...
    $contents = file_get_contents(
            $url, $use_include_path, $context, $offset, $maxLen);
...
$str, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) {
    $dom = new simple_html_dom(
            null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
foreach ($nodes as $node) {
    ...
    if ($pass && $attributes !== '' && is_array($attributes) && !empty($attributes)) {
        foreach ($attributes as $a) {
            ...
            if (is_object($debug_object)) {
                $debug_object->debug_log(2, 'testing node: '
                        . $node->tag
                        . ' for attribute: '
                        . $att_name
                        . $att_expr
            ...
            if ($lowercase) {
                $check = $this->match(
                        $att_expr, strtolower($att_val), strtolower($nodeKeyValue), $att_case_sensitivity
                );
            } else {
                $check = $this->match(
                        $att_expr, $att_val, $nodeKeyValue, $att_case_sensitivity
                );
            }

            if (is_object($debug_object)) {
                $debug_object->debug_log(2, 'after match: '
                        . ($check ? 'true' : 'false')
                );
            }
...
preg_match_all(
        $pattern, trim($selector_string) . ' ', // Add final ' ' as pseudo separator
...
foreach ($matches as $m) {
    ...
    if ($m[4] !== '') {
        preg_match_all(
                "/\[@?(!?[\w:-]+)(?:([!*^$|~]?=)[\"']?(.*?)[\"']?)?(?:\s*?([iIsS])?)?\]/is", trim($m[4]), $attributes, PREG_SET_ORDER
        );
...
if (is_object($debug_object)) {
    $debug_object->debug_log(3, 'source charset: '
            . $sourceCharset
            . ' target charaset: '
            . $targetCharset
    );
...
if (isset($this->attr['style'])) {
    ...
    preg_match_all(
            '/([\w-]+)\s*:\s*([^;]+)\s*;?/', $this->attr['style'], $matches, PREG_SET_ORDER
    );
...
$str = null, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT, $options = 0) {
if ($str) {
    if (preg_match('/^http:\/\//i', $str) || is_file($str)) {
        $this->load_file($str);
    } else {
        $this->load(
                $str, $lowercase, $stripRN, $defaultBRText, $defaultSpanText, $options
        );
    }
}
...
function load(
$str, $lowercase = true, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT, $options = 0) {
global $debug_object;
...
$str, $lowercase = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) {
$this->clear();
...
if (function_exists('get_last_retrieve_url_contents_content_type')) {
    ...
    if ($success) {
        $charset = $matches[1];
        if (is_object($debug_object)) {
            $debug_object->debug_log(2, 'header content-type found charset of: '
                    . $charset
            );
        }
    }
...
if (empty($charset)) {
    ...
    if (!empty($el)) {
        ...
        if (is_object($debug_object)) {
            $debug_object->debug_log(2, 'meta content-type tag found'
                    . $fullvalue
            );
        }
...
if (empty($charset)) {
    ...
    if (function_exists('mb_detect_encoding')) {
        ...
        $charset = mb_detect_encoding(
                $this->doc . 'ascii', $encoding_list = array('UTF-8', 'CP1252')
        );
...
if ((strtolower($charset) == strtolower('ISO-8859-1')) || (strtolower($charset) == strtolower('Latin1')) || (strtolower($charset) == strtolower('Latin-1'))) {
    ...
    if (is_object($debug_object)) {
        $debug_object->debug_log(
                2, 'replacing ' . $charset . ' with CP1252 as its a superset'
        );
    }
...
do {
    ...
    if ($this->doc[$this->pos - 1] == '<') {
        ...
        $node->_[HDOM_INFO_TEXT] = substr(
                $this->doc, $begin_tag_pos, $this->pos - $begin_tag_pos - 1
        );
        $this->pos -= 2;
        $this->char = ( ++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
        $this->link_nodes($node, false);
...
$count = preg_match_all(
        $pattern, $this->doc, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE
);
...
function iplResultDataPass($iplMatchDataArr = []) {
$insertingData = json_encode($iplMatchDataArr);
print_r($insertingData);
$fp = fopen('./assets' . DIRECTORY_SEPARATOR . 'Json' . DIRECTORY_SEPARATOR . 'live_match_live_details.json', 'w');
fwrite($fp, $insertingData);
fclose($fp);
}
...
for ($i = 0; $i < $len; $i++) {
if (!empty($data[$i])) {
    if ($data[$i]['header']['type'] == "T20" || $data[$i]['header']['type'] == "ODI") {
        $matchListArr[$a]['id'] = $data[$i]['matchId'];
        $matchListArr[$a]['type'] = $data[$i]['header']['type'];
        $matchListArr[$a]['mNum'] = $data[$i]['header']['mnum'];
        $matchListArr[$a]['mchDesc'] = $data[$i]['header']['mchDesc'];
        $matchListArr[$a]['mchState'] = $data[$i]['header']['mchState'];
        $matchListArr[$a]['teamOneId'] = $data[$i]['team1']['id'];
        $matchListArr[$a]['teamTwoId'] = $data[$i]['team2']['id'];

        $matchListArr[$a]['matchName'] = $data[$i]['srs'];
        $matchListArr[$a]['title'] = $data[$i]['header']['startdt'];
        $matchListArr[$a]['grnd'] = $data[$i]['header']['grnd'];
        $matchListArr[$a]['mWon'] = $data[$i]['header']['status'];
        if (isset($data[$i]['miniscore'])) {
            $team1id = $data[$i]['team1']['id'];
            if ($data[$i]['miniscore']['batteamid'] == $team1id) {
                if (!empty($data[$i]['miniscore']['batteamscore'])) {
                    $matchListArr[$a]['team_one_scoure'] = $data[$i]['miniscore']['batteamscore'];
                    $matchListArr[$a]['team_one_over'] = $data[$i]['miniscore']['overs'];
                    $matchListArr[$a]['s1Status'] = "0";
                } else {
                    $matchListArr[$a]['team_one_scoure'] = $data[$i]['miniscore']['batteamscore'];
                    $matchListArr[$a]['team_one_over'] = $data[$i]['miniscore']['overs'];
                    $matchListArr[$a]['s1Status'] = "1";
                }
            } elseif ($data[$i]['miniscore']['bowlteamid'] == $team1id) {
                if (!empty($data[$i]['miniscore']['bowlteamscore'])) {
                    $matchListArr[$a]['team_one_scoure'] = $data[$i]['miniscore']['bowlteamscore'];
                    $matchListArr[$a]['team_one_over'] = $data[$i]['miniscore']['bowlteamovers'];
                    $matchListArr[$a]['s1Status'] = "0";
                } else {
                    $matchListArr[$a]['team_one_scoure'] = $data[$i]['miniscore']['bowlteamscore'];
                    $matchListArr[$a]['team_one_over'] = $data[$i]['miniscore']['bowlteamovers'];
            ...
            $team2id = $data[$i]['team2']['id'];
            if ($data[$i]['miniscore']['batteamid'] == $team2id) {
                if (!empty($data[$i]['miniscore']['batteamscore'])) {
                    $matchListArr[$a]['team_two_scoure'] = $data[$i]['miniscore']['batteamscore'];
                    $matchListArr[$a]['team_two_over'] = $data[$i]['miniscore']['overs'];
                    $matchListArr[$a]['s2Status'] = "0";
                } else {
                    $matchListArr[$a]['team_two_scoure'] = $data[$i]['miniscore']['batteamscore'];
                    $matchListArr[$a]['team_two_over'] = $data[$i]['miniscore']['overs'];
                    $matchListArr[$a]['s2Status'] = "1";
                }
            } elseif ($data[$i]['miniscore']['bowlteamid'] == $team2id) {
                if (!empty($data[$i]['miniscore']['bowlteamscore'])) {
                    $matchListArr[$a]['team_two_scoure'] = $data[$i]['miniscore']['bowlteamscore'];
                    $matchListArr[$a]['team_two_over'] = $data[$i]['miniscore']['bowlteamovers'];
                    $matchListArr[$a]['s2Status'] = "0";
                } else {
                    $matchListArr[$a]['team_two_scoure'] = $data[$i]['miniscore']['bowlteamscore'];
                    $matchListArr[$a]['team_two_over'] = $data[$i]['miniscore']['bowlteamovers'];
            ...
            $matchListArr[$a]['rrr'] = $data[$i]['miniscore']['rrr'];
            $matchListArr[$a]['crr'] = $data[$i]['miniscore']['crr'];
            $matchListArr[$a]['cprtshp'] = $data[$i]['miniscore']['cprtshp'];
            $matchListArr[$a]['prevOvers'] = $data[$i]['miniscore']['prevOvers'];
            $matchListArr[$a]['lWkt'] = $data[$i]['miniscore']['lWkt'];
            $matchListArr[$a]['oversleft'] = $data[$i]['miniscore']['oversleft'];

            $matchListArr[$a]['strFname'] = $data[$i]['miniscore']['striker']['fullName'];
            $matchListArr[$a]['strRuns'] = $data[$i]['miniscore']['striker']['runs'];
            $matchListArr[$a]['strBalls'] = $data[$i]['miniscore']['striker']['balls'];
            $matchListArr[$a]['strFours'] = $data[$i]['miniscore']['striker']['fours'];
            $matchListArr[$a]['strSixes'] = $data[$i]['miniscore']['striker']['sixes'];
            if ($data[$i]['miniscore']['striker']['runs'] != 0 && $data[$i]['miniscore']['striker']['balls'] != 0) {
                $matchListArr[$a]['strSr'] = number_format(round($data[$i]['miniscore']['striker']['runs'] / $data[$i]['miniscore']['striker']['balls']), 2);
            } else {
                $matchListArr[$a]['strSr'] = 0;
            }
//                    $matchListArr[$a]['strSr'] = $data[$i]['miniscore']['striker']['runs'] / $data[$i]['miniscore']['striker']['balls'];

            $matchListArr[$a]['nstrFname'] = $data[$i]['miniscore']['nonStriker']['fullName'];
            $matchListArr[$a]['nstrRuns'] = $data[$i]['miniscore']['nonStriker']['runs'];
            $matchListArr[$a]['nstrBalls'] = $data[$i]['miniscore']['nonStriker']['balls'];
            $matchListArr[$a]['nstrFours'] = $data[$i]['miniscore']['nonStriker']['fours'];
            $matchListArr[$a]['nstrSixes'] = $data[$i]['miniscore']['nonStriker']['sixes'];

            $matchListArr[$a]['bowlerFname'] = $data[$i]['miniscore']['bowler']['fullName'];
            $matchListArr[$a]['bowlerOvr'] = $data[$i]['miniscore']['bowler']['overs'];
            $matchListArr[$a]['bowlerMai'] = $data[$i]['miniscore']['bowler']['maidens'];
            $matchListArr[$a]['bowlerRn'] = $data[$i]['miniscore']['bowler']['runs'];
            $matchListArr[$a]['bowlerWc'] = $data[$i]['miniscore']['bowler']['wicket'];
            if ($data[$i]['miniscore']['bowler']['runs'] != 0 && $data[$i]['miniscore']['bowler']['overs'] != 0) {
                $matchListArr[$a]['bowlerEr'] = number_format(round($data[$i]['miniscore']['bowler']['runs'] / $data[$i]['miniscore']['bowler']['overs']), 2);
            } else {
                $matchListArr[$a]['bowlerEr'] = "0";
            }
            $matchListArr[$a]['nbowlerFname'] = $data[$i]['miniscore']['nsbowler']['fullName'];
            $matchListArr[$a]['nbowlerOvr'] = $data[$i]['miniscore']['nsbowler']['overs'];
            $matchListArr[$a]['nbowlerMai'] = $data[$i]['miniscore']['nsbowler']['maidens'];
            $matchListArr[$a]['nbowlerRn'] = $data[$i]['miniscore']['nsbowler']['runs'];
            $matchListArr[$a]['nbowlerWc'] = $data[$i]['miniscore']['nsbowler']['wicket'];
            $matchListArr[$a]['flag'] = "0";
        } else {
            $matchListArr[$a]['team_one_scoure'] = "0/0";
            $matchListArr[$a]['team_one_over'] = "0.0";
            $matchListArr[$a]['s1Status'] = "1";
            $matchListArr[$a]['team_two_scoure'] = "0/0";
            $matchListArr[$a]['team_two_over'] = "0.0";
            $matchListArr[$a]['s2Status'] = "1";

            $matchListArr[$a]['rrr'] = "0";
            $matchListArr[$a]['crr'] = "0";
            $matchListArr[$a]['cprtshp'] = "0";
            $matchListArr[$a]['prevOvers'] = "";
            $matchListArr[$a]['lWkt'] = "";
            $matchListArr[$a]['oversleft'] = "";

            $matchListArr[$a]['strFname'] = "";
            $matchListArr[$a]['strRuns'] = "-";
            $matchListArr[$a]['strBalls'] = "0";
            $matchListArr[$a]['strFours'] = "0";
            $matchListArr[$a]['strSixes'] = "0";
            $matchListArr[$a]['strSr'] = "0";
            $matchListArr[$a]['nstrFname'] = "-";
            $matchListArr[$a]['nstrRuns'] = "0";
            $matchListArr[$a]['nstrBalls'] = "0";
            $matchListArr[$a]['nstrFours'] = "0";
            $matchListArr[$a]['nstrSixes'] = "0";

            $matchListArr[$a]['bowlerFname'] = "-";
            $matchListArr[$a]['bowlerOvr'] = "0";
            $matchListArr[$a]['bowlerMai'] = "0";
            $matchListArr[$a]['bowlerRn'] = "0";
            $matchListArr[$a]['bowlerWc'] = "0";
            $matchListArr[$a]['bowlerEr'] = "0";

            $matchListArr[$a]['nbowlerFname'] = "-";
            $matchListArr[$a]['nbowlerOvr'] = "0";
            $matchListArr[$a]['nbowlerMai'] = "0";
            $matchListArr[$a]['nbowlerRn'] = "0";
            $matchListArr[$a]['nbowlerWc'] = "0";
            $matchListArr[$a]['flag'] = "1";
        }
        ...
        $matchListArr[$a]['team_one_name'] = $data[$i]['team1']['name'];

        $matchListArr[$a]['team_two_name'] = $data[$i]['team2']['name'];
        $a++;
    }
}
}
//insert data from recent_match json file
iplResultDataPass(["Live" => $matchListArr]);
}
